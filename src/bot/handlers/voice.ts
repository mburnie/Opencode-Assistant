...
export async function handleVoiceMessage(ctx: Context, deps: VoiceMessageDeps): Promise<void> {
  ...
  try {
    ...
    // Show or hide the recognized text depending on STT_HIDE_RECOGNIZED_TEXT.
    // IMPORTANT: even if the edit/delete fails, we still send the text to OpenCode.
    if (config.stt.hideRecognizedText) {
      // Delete the "recognizing..." status message silently
      try {
        await ctx.api.deleteMessage(ctx.chat!.id, statusMessage.message_id);
      } catch (deleteError) {
        logger.debug("[Voice] Could not delete STT status message:", deleteError);
      }
    } else {
      try {
        await ctx.api.editMessageText(
